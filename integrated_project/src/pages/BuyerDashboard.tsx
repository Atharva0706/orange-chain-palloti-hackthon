...
import {
  ...
  CreditCard, CheckCircle2, Truck, Package, ArrowRight,
  AlertCircle, RefreshCw, ExternalLink, Clock
} from 'lucide-react'
...
const BuyerDashboard = () => {
  ...
  // ── Buy: create transaction + initiate escrow ────────────────────────────
  const handleBuy = async () => {
    ...
    try {
      ...
      setBuyMsg('✅ Escrow created! Depositing funds...')
      // Auto-advance to Deposited
      await fetch(`${API}/blockchain/status/${data.transaction._id}`, {
        method: 'PUT', headers: getHeaders(),
        body: JSON.stringify({ status: 'Deposited' })
      })
      await fetchProduce()
      await fetchMyTxs()
      setTimeout(() => { setShowEscrow(false); setSelectedLot(null); setBuyMsg(''); setActiveTab('orders') }, 1500)
    } catch { setBuyMsg('Server error. Please try again.') }
    finally { setBuyLoading(false) }
  }

  // ── Advance escrow step ──────────────────────────────────────────────────
  const advanceStatus = async (txId: string, currentStatus: string) => {
    const stepKeys = ESCROW_STEPS.map(s => s.key)
    const currIdx  = stepKeys.indexOf(currentStatus)
    if (currIdx === -1 || currIdx >= stepKeys.length - 2) return

    // If advancing to "Delivered" → call blockchain confirm endpoint
    if (stepKeys[currIdx + 1] === 'Delivered') {
      return confirmDelivery(txId)
    }

    setActionLoading(txId)
    setActionMsg(null)
    try {
      const res = await fetch(`${API}/blockchain/status/${txId}`, {
        method: 'PUT', headers: getHeaders(),
        body: JSON.stringify({ status: stepKeys[currIdx + 1] })
      })
      if (res.ok) {
        setActionMsg({ id: txId, msg: `Status updated to "${stepKeys[currIdx + 1]}"`, type: 'success' })
        await fetchMyTxs()
      } else {
        setActionMsg({ id: txId, msg: 'Failed to update status', type: 'error' })
      }
    } catch { setActionMsg({ id: txId, msg: 'Server error', type: 'error' }) }
    finally { setActionLoading(null) }
  }

  // ── Confirm delivery → mine block ────────────────────────────────────────
  const confirmDelivery = async (txId: string) => {
    setActionLoading(txId)
    setActionMsg(null)
    try {
      const res = await fetch(`${API}/blockchain/confirm/${txId}`, { method: 'POST', headers: getHeaders() })
      const data = await res.json()
      if (res.ok) {
        setActionMsg({
          id: txId,
  ...
  const canAdvance   = (status: string) => status !== 'Payment Released'
              ...
              return (
                <motion.div key={tx._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div style={{ padding: '1.5rem' }}>
                    ...
                    {/* Action button */}
                    {canAdvance(tx.escrowStatus) && (
                      <div style={{ display: 'flex', gap: '0.75rem' }}>
                        <button
                          onClick={() => advanceStatus(tx._id, tx.escrowStatus)}
                          disabled={actionLoading === tx._id}
                          style={{
                            padding: '0.75rem 1.5rem', borderRadius: '10px', fontWeight: 700, fontSize: '0.85rem',
                            background: tx.escrowStatus === 'In Transit' ? '#10b981' : 'var(--accent-orange)',
                            color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem',
                            opacity: actionLoading === tx._id ? 0.6 : 1
                          }}
                        >
                          {actionLoading === tx._id ? <RefreshCw size={14} className="spin" /> : <ArrowRight size={14} />}
                          {tx.escrowStatus === 'Pending Deposit' && 'Deposit Funds'}
                          {tx.escrowStatus === 'Deposited'       && 'Mark In Transit'}
                          {tx.escrowStatus === 'In Transit'      && '✅ Confirm Delivery (Mine Block)'}
                        </button>
                      </div>
                    )}
