import { LayoutDashboard, TrendingUp, BarChart2, Truck, Search, Wallet, ShoppingCart, LogOut, Link2 } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

interface SidebarProps { activeTab: string; setActiveTab: (tab: string) => void; userRole?: string; }

const farmerNav = [
  { id: 'farmer',      label: 'Dashboard',        icon: LayoutDashboard, color: '#10b981' },
  { id: 'forecast',    label: 'Price Forecast',   icon: TrendingUp,      color: '#f59e0b' },
  { id: 'market-intel',label: 'Market Intel',     icon: BarChart2,       color: '#ef4444' },
...
const Sidebar = ({ activeTab, setActiveTab, userRole }: SidebarProps) => {
  ...
  return (
    <aside style={{ width: 'var(--sidebar-width)', height: '100vh', background: '#0f172a', position: 'fixed', left: 0, top: 0, display: 'flex', flexDirection: 'column', padding: '0', zIndex: 100, borderRight: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <nav style={{ flex: 1, padding: '1.25rem 1rem', overflowY: 'auto' }}>
        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.85rem', padding: '0.8rem 1rem', borderRadius: '10px', border: 'none', marginBottom: '4px', background: isActive ? 'rgba(249,115,22,0.12)' : 'transparent', color: isActive ? 'var(--accent-orange)' : '#64748b', fontWeight: isActive ? 800 : 600, fontSize: '0.9rem', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s', borderLeft: isActive ? '3px solid var(--accent-orange)' : '3px solid transparent' }}>
